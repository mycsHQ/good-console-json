...
internals.utility = {
    formatOutput(event, settings) {
        ...
        const output = `hapijs timestamp="${timestamp}", eventId="${id}", tags="${tags}", ${event.data}`;
    ...
    formatResponse(event, tags, settings) {
        ...
        // method, pid, error
        const output = `eventInstance="${event.instance}", method="${method}", eventPath="${event.path}", query="${query}", statusCode="${statusCode}", responseTimeMs="${event.responseTime}"`;
    ...
    formatOps(event, tags, settings) {
        ...
        const output = `memoryMb="${memory}", uptimeSeconds="${event.proc.uptime}", load="[${event.os.load}]"`;
